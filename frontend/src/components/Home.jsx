...
//   className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center bg-purple-600 rounded-full hover:bg-purple-700 active:scale-95 transition-all"
// >
...
const Home = () => {
  ...
  const API_URL = "https://nasa-astronomy-appbackend.onrender.com/api/chat"; 

  const fetchApodData = async () => {
    try {
      const response = await fetch("https://nasa-astronomy-appbackend.onrender.com/api/apod");
      if (!response.ok) throw new Error("Failed to fetch APOD data");
      const data = await response.json();
      setApod(data);
    } catch (error) {
  ...
  <motion.div className="max-w-3xl text-center">
    ...
    {showDescription && (
      ...
      <div className="w-full sm:absolute sm:right-10 sm:bottom-10 sm:w-80 bg-black bg-opacity-80 backdrop-blur-lg p-4 rounded-lg shadow-xl z-50">
        ...
        {isTyping && <p className="text-gray-400 italic w-2/3">ChatGPT is typing...</p>}
        {/* ✅ Chat Input */}
     <div className="relative w-full mt-2">
  <input 
    type="text" 
